...
async fn main() -> Result<()> {
    ...
    if source == "bazaar_app" {
        ...
        println!("   ✅ Single-page category: {} products", single_page_products.len());
        ...
        println!("   ✅ Multi-page category: {} products", multi_page_products.len());
        
        // Test non-existent category (should handle gracefully)
        println!("\n3. Testing non-existent category: non-existent-category");
        let non_existent_products = fetcher.fetch_post_paginated("non-existent-category").await?;
        println!("   ✅ Non-existent category: {} products", non_existent_products.len());
        
        // Test category with special characters
        println!("\n4. Testing category with special characters: invalid@category#test");
        let special_char_products = fetcher.fetch_post_paginated("invalid@category#test").await?;
        println!("   ✅ Special characters category: {} products", special_char_products.len());
        
        println!("\n=== BazaarApp Pagination Test Summary ===");
        println!("Single-page (fruits-vegetables): {} products", single_page_products.len());
        println!("Multi-page (baby-care): {} products", multi_page_products.len());
        ...
        for (i, (category_name, url)) in test_categories.iter().enumerate() {
            ...
            println!("   ✅ Category '{}': {} products", category_name, products.len());
        }
        
        // Test invalid URL (should handle gracefully)
        println!("\n4. Testing invalid URL");
        let invalid_url = "https://k2products.kravemart.com/api/v2/es/categories/99999/products/1242164";
